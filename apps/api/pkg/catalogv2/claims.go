...
func (c *Client) CreateClaim(ctx context.Context, accessToken string, actor int, workID, siteWorkID int64) (*ClaimRecord, error) {
	body := map[string]any{"work_id": FormatID(workID)}
	if siteWorkID > 0 {
		body["site_work_id"] = FormatID(siteWorkID)
	}
	var out ClaimRecord
	if err := c.userPost(ctx, "/v2/me/claims", accessToken, actor, body, &out); err != nil {
		return nil, err
	}
	return &out, nil
}
...
func (c *Client) MintClaim(ctx context.Context, accessToken string, actor int, fieldValues map[string]any) (*ClaimRecord, error) {
	var out ClaimRecord
	if err := c.userPost(ctx, "/v2/me/claims", accessToken, actor,
