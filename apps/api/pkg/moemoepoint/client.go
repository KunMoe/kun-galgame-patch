...
func New(cfg Config) *Client {
	...
	return &Client{
		...
		// OAuth derives a ledger row's source_app from the authenticating client,
		// so an entry is "ours" iff its source_app equals our own client_id. The
		// moemoepoint balance is ecosystem-wide (forum / wiki / moyu share it), so
		// the log carries cross-app rows too; this lets the UI tell them apart.
...
type LogEntry struct {
	...
	// IsLocal is set by the client (NOT from OAuth): true when this row was
	// generated by moyu itself (source_app == our client_id). The UI uses it to
	// only deep-link our own refs and to badge cross-app (forum / wiki) rows.
	IsLocal bool `json:"is_local"`
	// Link is the in-site deep link for a local row, filled by the user service
	// (NOT from OAuth, NOT here — link-building needs patch-domain data, e.g.
	// resolving a comment id to its galgame). Empty when there's no linkable
	// target or the row is cross-app.
