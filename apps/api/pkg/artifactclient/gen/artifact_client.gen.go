...
// EnvelopeResumeUploadResponse defines model for EnvelopeResumeUploadResponse.
type EnvelopeResumeUploadResponse struct {
	// Schema A URL to the JSON Schema for this object.
	Schema  *string               `json:"$schema,omitempty"`
	Code    int64                 `json:"code"`
	Data    *ResumeUploadResponse `json:"data,omitempty"`
	Message string                `json:"message"`
}
...
// ResumeUploadResponse defines model for ResumeUploadResponse.
...
// UploadedPart defines model for UploadedPart.
...
type ClientInterface interface {
	...
	ResumeUpload(ctx context.Context, uuid string, reqEditors ...RequestEditorFn) (*http.Response, error)
}
...
func (c *Client) ResumeUpload(ctx context.Context, uuid string, reqEditors ...RequestEditorFn) (*http.Response, error) {
	req, err := NewResumeUploadRequest(c.Server, uuid)
	if err != nil {
		return nil, err
	}
	req = req.WithContext(ctx)
...
// NewResumeUploadRequest generates requests for ResumeUpload
func NewResumeUploadRequest(server string, uuid string) (*http.Request, error) {
	var err error
	...
	operationPath := fmt.Sprintf("/api/v1/artifacts/%s/resume", pathParam0)
	if operationPath[0] == '/' {
		operationPath = "." + operationPath
	}
...
type ClientWithResponsesInterface interface {
	...
	// ResumeUploadWithResponse request
	ResumeUploadWithResponse(ctx context.Context, uuid string, reqEditors ...RequestEditorFn) (*ResumeUploadResp, error)
}
...
type ResumeUploadResp struct {
	Body         []byte
	HTTPResponse *http.Response
	JSON200      *EnvelopeResumeUploadResponse
	JSONDefault  *HouseError
}

// Status returns HTTPResponse.Status
func (r ResumeUploadResp) Status() string {
	if r.HTTPResponse != nil {
		return r.HTTPResponse.Status
	}
	return http.StatusText(0)
...
// StatusCode returns HTTPResponse.StatusCode
func (r ResumeUploadResp) StatusCode() int {
	if r.HTTPResponse != nil {
		return r.HTTPResponse.StatusCode
	}
	return 0
...
// ContentType is a convenience method to retrieve the Content-Type value from the HTTP response headers
func (r ResumeUploadResp) ContentType() string {
	if r.HTTPResponse != nil {
		return r.HTTPResponse.Header.Get("Content-Type")
	}
	return ""
...
// ResumeUploadWithResponse request returning *ResumeUploadResp
func (c *ClientWithResponses) ResumeUploadWithResponse(ctx context.Context, uuid string, reqEditors ...RequestEditorFn) (*ResumeUploadResp, error) {
	rsp, err := c.ResumeUpload(ctx, uuid, reqEditors...)
	if err != nil {
		return nil, err
	}
	return ParseResumeUploadResp(rsp)
}
...
// ParseResumeUploadResp parses an HTTP response from a ResumeUploadWithResponse call
func ParseResumeUploadResp(rsp *http.Response) (*ResumeUploadResp, error) {
	bodyBytes, err := io.ReadAll(rsp.Body)
	defer func() { _ = rsp.Body.Close() }()
	if err != nil {
		return nil, err
	...
	switch {
	case strings.Contains(rsp.Header.Get("Content-Type"), "json") && rsp.StatusCode == 200:
		var dest EnvelopeResumeUploadResponse
		if err := json.Unmarshal(bodyBytes, &dest); err != nil {
			return nil, err
		}
		response.JSON200 = &dest
