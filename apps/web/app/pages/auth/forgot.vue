...
// Password recovery is handled entirely by the 鲲 Galgame OAuth server (passwords
...
useKunSeoMeta({
  title: '忘记密码',
  description: '通过 鲲 Galgame OAuth 重置密码'
})

const config = useRuntimeConfig()
const forgotUrl = computed(
  () => `${(config.public.oauthWebUrl as string) || ''}/forgot`
)
</script>
      ...
      <template #header>
        <div class="flex flex-col gap-2 p-6">
          ...
          <p class="text-default-500 text-center text-sm">
            密码由 鲲 Galgame OAuth 统一管理，请前往重置
          </p>
        </div>
      </template>
