...
banner: '/posts/notice/cfmsc/banner.avif'
description: 'CFMSC 是鲲 Galgame 发布以及编辑资源流程 （Check -> Fetch -> Modify -> Submit Collaborate） 过程的首字母缩写'
date: 2024-12-08
authorUid: 2
authorName: '鲲'
...
#### 向我们的后端接口发起请求, 判断用户输入的 VNDB_ID 是否已经有人发过对应的游戏了 (Check Duplicate)

#### 若该游戏未重复, 则向 VNDB 的开放 API 接口发起请求, 请求部分我们需要用到的字段 (Fetching Resources)
...
#### 基本上所有的网站提供的介绍的数据, 都是没有预览图的, 然而大部分情况下用户是想要看到几张游戏预览图的

#### 倘若我要为《永不枯萎的世界与终结之花》这个游戏, 添加一个 `枯花` 的别名, 应该如何处理
...
#### 极强的可扩展性, 可以将任何图片, 乃知视频, 甚至是 DOM 元素插入 Galgame 的介绍, 实现完美而又细节的介绍

#### 极强的自定义性, 如果有用户不想看游戏预览图, 因为这很占用页面空间, 那么可以直接关闭预览图

#### 极强的可编程性, 如果要实现一个功能, 对该游戏的预览图进行分类, 全年龄的预览图标记为普通, R18 的预览图标记为 NSFW, 这可以很轻易的实现
...
对于鲲 Galgame 论坛, 新增的字符会被标记为蓝色, 删减的字符会被编辑为红色, 合并后文本会变为绿色, 拒绝后更新请求将不可被查看这个, 这个过程使用了鲲 Galgame 论坛 [自行编写的 diff 算法](https://github.com/KunMoe/kun-galgame-nuxt3)
