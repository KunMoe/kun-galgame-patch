...
The authoritative contract docs are synced as **read-only mirrors** under `docs/{oauth,image_service,galgame_wiki}/` (files carry a GENERATED banner in the header).
...
- `docs/{oauth,image_service,galgame_wiki}/` are all infra mirrors (including `image_service/03-api-design.md` — its early implementation corrections have been folded back into the infra source, and as of 2026-06 it is no longer a variant). To change them, change them in infra and then `docs:sync`; **do not touch the copies here**.
