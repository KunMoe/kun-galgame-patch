...
2. Browser redirects to `account.nextmoe.com/api/v1/oauth/authorize`
...
> If you previously created the `oauth_account` table, drop it. The `migrate-users` script in `nextmoe-infra/apps/api/cmd/migrate-users/` handles the case where the table doesn't exist (it filters tables via `pg_tables` before remapping FK columns).
