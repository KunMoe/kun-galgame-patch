...
- 关键路径自检：
  - `histRow` 切换不同实体时（用户切 tab 后再开历史）状态正确重置 ✅
  - revert 成功后既刷新 history 列表也刷新 entity 列表（被恢复的实体重新出现）✅
  - acting 状态防止并发点击多个 revert 按钮 ✅
  - 鉴权失败（普通用户点 revert）→ Wiki 返 403 → 透传 → useKunMessage 友好提示 ✅
...
- 自检：
  - 仅改 note 不写 history（fileChanged=false）✅
  - 换 storage / s3_key / content 任一 → 1 条 history 行（事务内先写后改）✅
  - 删除资源 → CASCADE 同删 history（删除即遗忘语义）✅
  - 普通用户 / admin / moderator 编辑各自记录正确的 actor_role ✅
  - admin UI「历史」按钮可拉取分页 + 弹窗展示 ✅
...
- 后端 zero 改动
- 手算验证算法正确：`trimSharedEdges('abc','aBc') = {prefix:'a', oldMid:'b', newMid:'B', suffix:'c'}` ✅；`diffLines('a\nb\nc','a\nB\nc')` → `[eq a, del b, add B, eq c]` ✅；`setDiff([1,2,3],[2,3,4])` → `{added:[4], removed:[1], kept:[2,3]}` ✅
...
- 自检：
  - 提交带前缀 `patch/123/...` 的 s3_key → 通过路径校验 ✅
  - 提交 `secrets/foo` 等 → 入 service 立即拒绝（早于 DB） ✅
  - 同一 s3_key 创建两次 patch_resource → 第二次 DB unique violation → 友好提示 ✅
  - 同一 s3_key 调 CompleteSmall 两次 → 首次正常扣配额，第二次 SETNX 命中跳过扣减（idempotent）✅
