...
| 页面类型 | 用户身份 | NSFW 模式 | 后端返回 | SEO 决策 |
|---|---|---|---|---|
| Patch / Resource 详情 | 匿名（爬虫） | sfw | 404 | useKunDisableSeo（NSFW 确认 placeholder） |
| Patch / Resource 详情 | 匿名 + 已 ack | sfw | 200 + NSFW 数据 | useKunDisableSeo（数据可见但不索引） |
| Patch / Resource 详情 | 匿名 | nsfw/all | 200 + NSFW 数据 | useKunDisableSeo（同上） |
| Patch / Resource 详情 | 登录 | * | 200 + 任意数据 | 视 content_limit：SFW → useKunSeoMeta；NSFW → useKunDisableSeo |
| 列表页（home / galgame / resource / comment / ranking） | * | * | SFW 子集（后端 filter） | useKunSeoMeta（payload 已过滤 NSFW） |
| 搜索页 | * | * | 全集（含 NSFW） | useKunDisableSeo（避免动态 URL + NSFW 暴露） |
| Tag / Official / Engine / Series 详情 | * | * | wiki 端默认 sfw filter | useKunSeoMeta（loaded） / useKunDisableSeo（404） |
...
3. **NSFW 检查**：
   ...
   - 该页是否走 useApi？useApi 已综合判断（登录 / cookie / ack）决定 content_limit，**不要**在 page 内再做客户端 NSFW 过滤（违反 wiki §16.6 协议）
